...
<div class="cztable">
	<form action="/power/menu/updateMenu" method="post">
        ...
        <table border="1" width="100%" class="table_a">
				...
				<tr  width="120px;">
                    ...
                    <td>
                    	<select name="upmenuid">
                            <option <c:if test="${menu.upmenuid==-1}">selected</c:if> value="-1">顶级菜单</option>
                            <c:forEach items="${menuList}" var="menu1">
                                <option <c:if test="${menu1.menuid==menu.upmenuid}">selected</c:if> value="${menu1.menuid}">${menu1.menuname}</option>
                            </c:forEach>
                        </select>
					</td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <input <c:if test="${menu.menustate==1}">checked</c:if> type="radio" name="menustate"  value="1" />启用
                        <input <c:if test="${menu.menustate==0}">checked</c:if> type="radio" name="menustate" value="0"/>禁用
                    </td>
                </tr>
