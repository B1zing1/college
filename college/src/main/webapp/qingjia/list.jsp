        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <c:forEach items="${leaveBillList}" var="leavebill">
                <tr style="height: 25px" align="center">
                    ...
                    <th scope="col">${userTb1.userRealname}</th>
                    <th scope="col">${leavebill.days}</th>
                    <th scope="col">${leavebill.content}</th>
                    <th scope="col">${leavebill.remark}</th>
                    <th scope="col">${leavebill.date}</th>
                    <th scope="col">
                        <c:if test="${leavebill.state==0}">初始录入</c:if>
                        <c:if test="${leavebill.state==1}">开始审批</c:if>
                        <c:if test="${leavebill.state==2}">审批完成</c:if>
                        <c:if test="${leavebill.state==3}">未通过</c:if>
                    </th>
                    <th scope="col">
                        <c:if test="${leavebill.state==0}">修改 删除
                            <a href="/qingjia/submit?leaveid=${leavebill.id}">提交申请</a>
                        </c:if>
                        <c:if test="${leavebill.state==1}">查看</c:if>
                        <c:if test="${leavebill.state==2}">查看 删除</c:if>
                        <c:if test="${leavebill.state==3}">修改 删除
                            <a href="/qingjia/submit?leaveid=${leavebill.id}">提交申请</a>
                        </c:if>
                    </th>
                </tr>
