    ...
    <script>
        ...
        $(function () {
            $("[name=num]").change(function () {
                var size = $(this).val();
                location.href = "/power/role/findAllRole?size=" + size;
            });
        })
    </script>
</head>
...
<div class="div_head" style="width: 100%;text-align:center;">
		<span> <span style="float:left">当前位置是：权限管理-》角色管理</span> <span
			...
			<a style="text-decoration: none;" href="/power/role/findAllMenu">【新增角色】</a>&nbsp;&nbsp;&nbsp;&nbsp;
		</span>
		</span>
</div>
<div class="morebt">
...
<div class="cztable">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <c:forEach items="${pageInfo.list}" var="role" varStatus="sta">
            <tr align="center">
                <td>
                    ${sta.count}
                </td>
                ...
                <td>&nbsp;
                    <c:if test="${role.roleid!=userTb1.role.roleid}">
                        <a href="/power/role/changeState?rolestate=${role.rolestate==1?0:1}&roleid=${role.roleid}">${role.rolestate==1?'禁用':'启用'}</a>
                    </c:if>
                    <a href="/power/role/findRoleById?roleid=${role.roleid}">详情</a>
                    <a href="/power/role/findRoleToUpdate?roleid=${role.roleid}">修改</a>
                    <c:if test="${role.roleid!=userTb1.role.roleid}">
                     <a href="/power/role/deleteRole?roleid=${role.roleid}" onclick="return del();return false" class="tablelink"> 删除</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
        <tr align="center">
            <td colspan="20" style="text-align: center;">
                <doPage:fy url="/power/role/findAllRole" pageInfo="${pageInfo}"></doPage:fy>
            </td>
        </tr>
        </tbody>
    </table>
